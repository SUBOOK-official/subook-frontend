...
function StepAddress({ address, setAddress, savedAddresses, onNext, showToast }) {
...
// ─── Step 3: 확인 및 제출 ───
function StepConfirm({ address, account, isSubmitting, onPrev, onSubmit, goToStep }) {
  ...
  return (
    <div className="pickup-success">
      ...
      <div className="pickup-success__info">
        ...
        <p>예상 {expectedCount}권 · 박스 {boxCount}개</p>
      </div>
...
function PublicPickupRequestPage() {
  usePageMeta({
    title: "교재 위탁판매 신청",
    description: "안 쓰는 수능 교재를 CJ 픽업으로 보내고 검수 후 정산까지. 예상 권수와 박스 수만 알려주면 끝.",
  });
  ...
  useEffect(() => {
    ...
    if (draft && draft.address && (draft.address.recipient_name || draft.address.address_line1 || draft.address.expected_book_count)) {
  ...
  useEffect(() => {
    ...
    if (!address.address_line1 && !address.recipient_name && !address.expected_book_count) return;
  ...
  const handleDraftContinue = () => {
    ...
    if (draftPrompt.policyAgreed && typeof draftPrompt.policyAgreed === "object") {
      setPolicyAgreed((prev) => ({ ...prev, ...draftPrompt.policyAgreed }));
    }
    if (Number.isFinite(draftPrompt.step)) setCurrentStep(Math.min(2, Math.max(0, draftPrompt.step)));
    setDraftPrompt(null);
    draftDecidedRef.current = true;
    draftReadyRef.current = true;
  };
  ...
  const handleCancel = () => {
    if (address.address_line1 || address.recipient_name || address.expected_book_count) {
        ...
        <main className="pickup-route">
          <div className="pickup-shell">
            <PickupSuccess
              ...
              expectedCount={Number.parseInt(address.expected_book_count, 10) || 0}
              result={submitResult}
            />
          </div>
        </main>

...
## 통신판매업·에스크로·도메인 순서 (행정 — 2026-06-21 확인)

상황: subook.kr은 현재 옛 식스샵 사이트. 자체제작 새 사이트는 임시 `subook-public-web-temp.vercel.app`. 아직 버그/미완성이라 subook.kr 컷오버 못 함.

- 통신판매업 신고서에 등록되는 건 **도메인(subook.kr) + 호스트서버 소재지** — 바뀌면 15일 내 **변경신고** 의무 ([소비자24](https://www.consumer.go.kr/user/bbs/consumer/380/940/bbsDataView/3183.do) / [정부24](https://www.gov.kr/mw/AA020InfoCappView.do?CappBizCD=11300000007)).
- → **vercel.app 말고 처음부터 subook.kr로 신청.** 사이트 내용물(식스샵→자체제작)이 바뀌는 건 신고 항목이 아님.
- 컷오버 전에도: **은행 에스크로**(국민/기업/농협/우체국 사업자계좌, 도메인 무관) + **통신판매업**은 subook.kr로 지금 신청 가능. **토스 전자결제 가맹심사는 실제 라이브 사이트를 검수**하므로 컷오버(subook.kr→새 사이트) 후 또는 vercel.app URL 제출.
- 에스크로 루트: 은행(사업자 기준, 즉시) vs 토스(전자결제 가입 시 함께, 제출 URL에 묶임). 토스 "통신판매업 바로신청"은 토스 가맹=URL에 묶이므로 컷오버 후 사용 권장.
- ⚠ 호스트서버 소재지(식스샵→Vercel) 변경이 변경신고 대상인지는 관할 구청에 확인 권장(지자체별 운영 차이).
- 최종 순서: **사이트 완성 → subook.kr DNS 컷오버 → 토스 가맹심사 → 라이브 키로 PG ON.** 그 전까지 은행 에스크로+통신판매업은 병렬 진행 가능.

...
export default function IntakeCamera({ onCapture, disabled, label }) {
  ...
  const capture = useCallback(async () => {
    ...
    if (disabled || !connected || !video?.videoWidth) return;
    ...
    if (blob) onCapture(new File([blob], `czur-${Date.now()}.jpg`, { type: 'image/jpeg' }));
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4" aria-label="CZUR 카메라">
    ...
    <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-lg bg-slate-950">
    ...
    <button type="button" onClick={capture} disabled={!connected || disabled} className="mt-3 w-full rounded-lg bg-blue-700 px-4 py-3 font-bold text-white disabled:opacity-40">{label} <span className="ml-2 text-xs font-normal">Space</span></button>
    <label className={`mt-3 block cursor-pointer text-center text-sm text-slate-600 underline ${disabled ? 'opacity-40' : ''}`}>사진 파일 선택<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={disabled} onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ''; if (file) onCapture(file); }} /></label>
  </section>;
}

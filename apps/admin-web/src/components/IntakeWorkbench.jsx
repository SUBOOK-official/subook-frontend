...
import { blankIntake, INTAKE_STEPS, intakeError, intakePayload } from '../lib/intakeWorkbench';
...
function readWorkspace(key) {
  try {
    ...
    if (draft?.version === 1 && draft.active?.requestKey && Array.isArray(draft.held)) return draft;
...
export default function IntakeWorkbench({ shipment, onChangeCustomer, legacyDraft }) {
  ...
  useEffect(() => {
    ...
    supabase.rpc('admin_intake_price_context', { p_item: {
      product_id: item.product_id, title: item.title, option: item.option, condition_grade: item.condition_grade,
    ...
    return () => { active = false; };
  }, [item.step,item.product_id,item.title,item.option,item.condition_grade,item.subject,item.brand,item.book_type,item.published_year,priceRetry]);
  ...
  async function submit() {
    ...
    try {
      const { data, error } = await supabase.rpc('admin_register_intake_book', {
        p_shipment_id: shipment.id, p_request_key: pending.requestKey, p_item: pending.item,
      });
      if (error) {
  ...
  return <AdminShell activeModule="register" title="상품 등록" description="한 권씩 촬영하고 확인하는 등록 작업대">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-5 py-4">
      <div><p className="text-xs font-semibold text-slate-500">현재 수거 건</p><p className="mt-1 font-bold text-slate-900">{shipment.seller_name} <span className="ml-2 text-sm font-normal text-slate-500">{shipment.pickup_date} · 등록 {workspace.completed.length}권 · 보류 {workspace.held.length}권</span></p></div>
    ...
    {lastResult ? <div role="status" className="mb-4 flex items-center justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4"><div><p className="font-bold text-emerald-800">{lastResult.discarded ? '판매불가 기록 완료' : '등록 완료'} · 일련번호 <span className="text-xl">{lastResult.serial_number}</span></p><p className="mt-1 text-sm text-emerald-800">{lastResult.title} · {lastResult.location || '위치 미지정'} · {lastResult.discarded ? '비공개' : lastResult.is_public ? '스토어 공개' : '비공개'}</p></div><button type="button" onClick={() => setLastResult(null)} aria-label="등록 완료 알림 닫기" className={secondary}>확인</button></div> : null}
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(320px,0.85fr)_minmax(0,1.15fr)]">
      ...
      <section className="min-w-0 rounded-xl border border-slate-200 bg-white p-5 sm:p-6" aria-label="현재 교재 작업">
        ...
        <fieldset disabled={locked} className="mt-5 min-w-0 space-y-5 disabled:opacity-60">
          {item.step===0 ? <>
            <p className="text-sm text-slate-600">표지가 화면에 꽉 차도록 놓고 촬영하세요.</p>
          ...
          {item.step===1 ? <>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex items-center justify-between"><span className="text-xs font-bold text-blue-700">{item.product_id ? '기존 교재에 재고 추가' : '신규 교재'}</span>{item.product_id ? <button type="button" onClick={() => patch({ product_id: null, options: [], price: '', cover_image_url: item.scan_image_url || '' })} className="text-xs underline">신규 교재로 변경</button> : null}</div>
              <Field label="교재명"><input className={inputClass} value={item.title} disabled={Boolean(item.product_id)} onChange={(event) => patch({ title: event.target.value })} placeholder="학년도·회차가 구분되는 정확한 교재명" /></Field>
              ...
              <div className="mt-4"><Field label="옵션·구성 (낱권·회차·세트)"><input list="intake-options" className={inputClass} value={item.option} onChange={(event) => patch({ option: event.target.value, price: '' })} placeholder="기본 구성은 비워두세요" /><datalist id="intake-options">{item.options.map((option,index) => <option key={index} value={option.option || ''} />)}</datalist></Field><p className="mt-2 text-xs text-slate-500">한 번 등록하면 재고 1개가 생성됩니다. 세트는 구성 전체를 확인하세요.</p></div>
            </div>
          </> : null}
          {item.step===2 ? <>
            <p className="font-semibold">{item.title} {item.option ? `· ${item.option}` : ''}</p>
            <div><p className="mb-2 text-sm font-semibold">등급</p><div className="grid grid-cols-2 gap-2">{Object.entries(bookConditionLabel).map(([grade,label]) => <button type="button" key={grade} aria-pressed={item.condition_grade===grade} onClick={() => patch({ condition_grade: grade })} className={`rounded-lg border px-3 py-3 text-sm font-semibold ${item.condition_grade===grade ? 'border-blue-700 bg-blue-50 text-blue-700' : 'border-slate-300'}`}>{grade==='DISCARD' ? '판매불가' : label}</button>)}</div></div>
            {item.condition_grade==='DISCARD' ? <Field label="판매불가 사유"><textarea className={inputClass} value={item.discard_reason} onChange={(event) => patch({ discard_reason: event.target.value })} placeholder="필기 과다, 페이지 누락 등" /></Field> : <>
              ...
              <label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={item.components_confirmed} onChange={(event) => patch({ components_confirmed: event.target.checked })} />답지·회차·세트 구성품을 확인했습니다</label>
            </>}
            ...
            <div><p className="text-sm font-semibold">내지 사진 {item.inspection_image_urls.length}/{MAX_DETAIL_PHOTOS}</p><div className="mt-3 grid grid-cols-2 gap-3">{item.inspection_image_urls.map((url,index) => <div key={url} className="rounded-lg border border-slate-200 p-2"><img src={url} alt={`내지 사진 ${index+1}`} className="h-36 w-full object-contain" /><button type="button" className="mt-2 w-full text-xs text-rose-700 underline" onClick={() => patch({ inspection_image_urls: item.inspection_image_urls.filter((_,i) => i!==index) })}>삭제 후 다시 촬영</button></div>)}</div><p className="mt-2 text-xs text-slate-500">왼쪽 카메라에서 내지를 촬영하면 현재 책에 연결됩니다.</p></div>
          </> : null}
          {item.step===3 ? <>
            <p className="font-semibold">{item.title} · {bookConditionLabel[item.condition_grade]} {item.option ? `· ${item.option}` : ''}</p>
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-4"><Field label="판매가 (원)"><input type="number" min="1" className={`${inputClass} text-xl font-bold`} value={item.price} onChange={(event) => patch({ price: event.target.value })} placeholder="비교 가격을 선택하거나 직접 입력" /></Field>{prices?.recommended_price ? <div className="mt-3 flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-blue-700">동일 옵션·등급의 최근 판매가 중앙값 <strong>{formatCurrency(prices.recommended_price)}</strong></p><button type="button" className={secondary} onClick={() => patch({ price: String(prices.recommended_price) })}>이 가격 적용</button></div> : null}</div>
          ...
          {item.step===4 ? <>
            <div className="flex gap-4 rounded-lg bg-slate-50 p-4">{item.cover_image_url ? <img src={item.cover_image_url} alt="등록할 표지" className="h-32 w-24 object-contain" /> : null}<div><p className="font-bold">{item.title}</p><p className="mt-2 text-sm text-slate-600">{item.option || '기본 구성'} · {bookConditionLabel[item.condition_grade]}</p><p className="mt-2 text-xl font-bold">{item.condition_grade==='DISCARD' ? '판매불가' : formatCurrency(Number(item.price))}</p><p className="mt-2 text-xs text-slate-500">내지 사진 {item.inspection_image_urls.length}장</p></div></div>
            {item.condition_grade!=='DISCARD' ? <>
              <div className="grid gap-3 sm:grid-cols-2"><Field label="보관 위치"><input className={inputClass} value={item.location} onChange={(event) => patch({ location: event.target.value })} placeholder="예: A-2" /></Field><Field label="일련번호 (비우면 자동 배정)"><input type="number" min="1" className={inputClass} value={item.serial_number} onChange={(event) => patch({ serial_number: event.target.value })} /></Field></div>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4">
          <div className="flex gap-2">{item.step>0 ? <button type="button" disabled={locked} className={secondary} onClick={() => patch({ step: item.step===4 && item.condition_grade==='DISCARD' ? 2 : item.step-1 })}>이전</button> : null}<button type="button" disabled={locked} className={secondary} onClick={hold}>보류·다음 책</button></div>
          {item.step===4 ? <button type="button" disabled={locked} onClick={submit} className={primary}>{item.condition_grade==='DISCARD' ? '판매불가 기록·다음 책' : '등록·다음 책'}</button> : item.step>0 || item.scan_image_url ? <button type="button" disabled={locked} onClick={next} className={primary}>{item.step===2 ? '확인·가격 결정' : '확인·다음'}</button> : null}
        </div>
